...
dg.populate = function()
{
	...
	for (i = dhcpd_lease.length - 1; i >= 0; --i) {
		a = dhcpd_lease[i];
		e = get(a[2], a[1]);
		e.lease = '<small><a href="javascript:deleteLease(\'L' + i + '\',\'' + a[1] + '\')" title="删除固定 DHCP" id="L' + i + '">' + a[3] + '</a></small>';
		e.name = a[0];
		e.ifname = '';
	}
	...
	for (i = list.length - 1; i >= 0; --i) {
		...
		if (e.mac.match(/^(..):(..):(..)/)) {
			b += '<br><small>' +
				'<a href="http://api.macvendors.com/' + RegExp.$1 + '-' + RegExp.$2 + '-' + RegExp.$3 + '" target="_new" title="OUI 检索">[oui]</a> ' +
				'<a href="javascript:addStatic(' + i + ')" title="加入静态 DHCP 租用...">[绑定]</a> ' +
				'<a href="javascript:addbwlimit(' + i + ')" title="加入宽带限速">[限速]</a>';

			if (e.rssi != '') {
				b += ' <a href="javascript:addWF(' + i + ')" title="加入无线客户端过滤清单...">[过滤]</a>';
			}
			b += '</small>';
		}
		else {
