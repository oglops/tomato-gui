...
tabs = [['client1', 'VPN客户端1'],['client2', 'VPN客户端2']];
...
for (i = 0; i < tabs.length; ++i)
{
	...
	createFieldTable('', [
	    ...
	    W('<div>');
		W('<ul>');
			W('<li><b>选择类型 -> 从源IP</b> - 例: "1.2.3.4" or "1.2.3.0/24".');
			W('<li><b>选择类型 -> 到目的IP</b> - 例: "1.2.3.4" or "1.2.3.0/24".');
			W('<li><b>选择类型 -> 到域名</b> - 例: "domain.com". 一行输入一个域名');
		W('</ul>');
	    W('</div>');
	W('</div>');
	W('<div id=\''+t+'-keys\'>');
