	...
	if (daily_history.length > 0) {
		...
		for (i = 0; i < daily_history.length; ++i) {
			...
			if (E('_f_shortcuts').checked) {
				...
				h = h + '<a href="javascript:viewIptDetail(' + i + ')" title="查看此IP实时流量">[iptraf]</a>';
				h = h + '<a href="javascript:addExcludeList(' + i + ')" title="过滤此IP地址">[hide]</a>';
				h = h + '</small>';
			}
			var ymd = getYMD(b[0]);
...
<div id='cstats'>
<div class='section-title'>IP 流量监控 - 每日流量</div>
<div class='section'>
...
c.push({ title: '忽略没有流量的IP', name: 'f_ignorezeroes', type: 'checkbox' });
